...
def verboser(iptFunc:typing.Optional[typing.Callable[[F], F]]= None) -> F:
	...
	def inner(*args, **kwargs) -> F:
		...
		pidPrint(f"{iptFunc.__name__}: starting execution at @ {datetime.now()} with the following arguments:")
		pidPrint(f"\twith the following arguments:")
		for arg in args:pidPrint(f"\t\t{iptFunc.__name__}: {arg}\t{type(arg)}")

		pidPrint(f"\t{iptFunc.__name__}: with the following keyword arguments:")
		for kwarg,value in kwargs.items():pidPrint(f"\t\t{iptFunc.__name__}: {kwarg.ljust(12, ' ')}=\t{value}\t{type(value)}")

		start_timer		= perf_counter()
		returnValue		= iptFunc(*args, **kwargs)
		executionTime	= perf_counter() - start_timer
		pidPrint(f"{iptFunc.__name__}: finished execution at @ {datetime.now()}, took {executionTime} s.")
		#pidPrint(f"{iptFunc.__name__}: returnValue = {returnValue}: {type(returnValue)}.\n")
		return returnValue
	return inner